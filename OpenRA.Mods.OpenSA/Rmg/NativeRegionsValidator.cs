...
namespace OpenRA.Mods.OpenSA.Rmg
{
	public static partial class NativeMovementValidator
	{
		static IEnumerable<CPos> NativeStartingPositions(RmgGenerationResult generation) =>
			generation.Settings.GeneratorVersion is 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 ? generation.Map.Actors.Where(a => a.Role == "start").Select(a =>
			{
		...
		static RmgNativeMovementValidationResult ValidateRegions(Map map, RmgGenerationResult generation)
		{
			...
			if (settings.GeneratorVersion is 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25)
			{
				...
				Require(colonies.All(colony => allowed.Contains(colony.Type) || (settings.GeneratorVersion is 24 or 25 && colony.Type == "wasps_colony" && mandatory.Contains(colony.Location))), "COLONY_WEIGHTS", "Saved map contains an excluded neutral colony type.");
			...
			if (settings.GeneratorVersion is 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25)
			{
			...
			if (settings.GeneratorVersion == 25) ValidateChaos(map, generation, withStarts, grid, starts, colonies, result);
			...
			result.RegionsPolicy["accessibility_requirement"] = settings.GeneratorVersion is 24 or 25 ? "WASPS_ACCESS_ON_EVERY_ISLAND" : settings.GeneratorVersion == 22 ? "STARTS_AND_COLONIES_CONNECTED" : settings.GeneratorVersion == 21 ? "CONNECTED_HOME_TERRITORIES" : settings.GeneratorVersion is 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 ? "STARTS_AND_COLONIES_CONNECTED" : "NOT_REQUIRED";
			result.RegionsPolicy["strategic_routes_requirement"] = settings.GeneratorVersion == 25 ? "UNCONSTRAINED_EXCEPT_LOCAL_NEST_ACCESS" : settings.GeneratorVersion == 24 ? "DISCONNECTED_ISLANDS" : settings.GeneratorVersion == 23 ? "CONNECTED_WINDING_PASSAGES" : settings.GeneratorVersion == 22 ? "CONNECTED_FORT_ENTRANCES" : settings.GeneratorVersion == 21 ? (settings.LandCrossings == RmgLandCrossings.None ? "DISCONNECTED_TERRITORIES" : "EXACT_BORDER_CROSSINGS") : settings.GeneratorVersion is 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 ? "CONNECTED_CLEAR_LANE_NETWORK" : "NOT_REQUIRED";
			result.RegionsPolicy["flying_unit_availability_requirement"] = settings.GeneratorVersion is 24 or 25 ? "MANDATORY_NEUTRAL_WASPS_PER_ISLAND" : "NOT_REQUIRED";
