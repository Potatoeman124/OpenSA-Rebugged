...
namespace OpenRA.Mods.OpenSA.Rmg
{
	public static partial class RmgGenerator
	{
		...
		public static RmgGenerationResult Generate(RmgProfile profile, RmgGenerationSettings settings)
		{
			...
			if (settings.GeneratorVersion == 25) return GenerateChaos(profile, settings);
			if (settings.GeneratorVersion == 24) return GenerateArchipelago(profile, settings);
			if (settings.GeneratorVersion == 23) return GenerateLabyrinth(profile, settings);
			if (settings.GeneratorVersion == 22) return GenerateStrongholds(profile, settings);
			if (settings.GeneratorVersion == 21) return GenerateDividedLands(profile, settings);
			if (settings.GeneratorVersion == 20) return GenerateRing(profile, settings);
			if (settings.GeneratorVersion == 19) return GenerateCrossroads(profile, settings);
			if (settings.GeneratorVersion == 18) return GenerateArtificialBattlefield(profile, settings);
		...
		{
			if (settings.GeneratorVersion == 18) RmgBattlefieldParameters.Validate(settings);
			if (settings.GeneratorVersion == 21) RmgDividedLandsParameters.Validate(settings);
			if (settings.GeneratorVersion == 20) RmgRingParameters.Validate(settings);
			if (settings.GeneratorVersion == 19) RmgCrossroadsParameters.Validate(settings);
			if (settings.GeneratorVersion is 17 or 18 or 19 or 20 or 21) RmgMirroring.Validate(settings.MirroringAxes, settings.MapSize, settings.PlayerCount);
			...
			if (!RmgBiome.IsSupported(settings.Tileset) || settings.Tileset != profile.Tileset ||
				(settings.Tileset != "NORMAL" && (settings.GeneratorVersion is not (16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25) || !profile.UsesRegionsTerrain)))
			...
			if (profile.UsesRegionsTerrain && (settings.TopologyPreset != (settings.GeneratorVersion == 25 ? RmgTopologyPreset.Chaos : settings.GeneratorVersion == 24 ? RmgTopologyPreset.Archipelago : settings.GeneratorVersion == 23 ? RmgTopologyPreset.Labyrinth : settings.GeneratorVersion == 22 ? RmgTopologyPreset.Strongholds : settings.GeneratorVersion == 21 ? RmgTopologyPreset.DividedLands : settings.GeneratorVersion == 20 ? RmgTopologyPreset.Ring : settings.GeneratorVersion == 19 ? RmgTopologyPreset.Crossroads : settings.GeneratorVersion == 18 ? RmgTopologyPreset.PlannedBattlefield : RmgTopologyPreset.NaturalRegions) ||
				settings.LayoutFamily != (settings.GeneratorVersion == 25 ? RmgLayoutFamily.Chaos : settings.GeneratorVersion == 24 ? RmgLayoutFamily.Archipelago : settings.GeneratorVersion == 23 ? RmgLayoutFamily.Labyrinth : settings.GeneratorVersion == 22 ? RmgLayoutFamily.Strongholds : settings.GeneratorVersion == 21 ? RmgLayoutFamily.DividedLands : settings.GeneratorVersion == 20 ? RmgLayoutFamily.Ring : settings.GeneratorVersion == 19 ? RmgLayoutFamily.Crossroads : settings.GeneratorVersion == 18 ? RmgLayoutFamily.ArtificialBattlefield : settings.GeneratorVersion == 17 ? RmgLayoutFamily.NaturalLandscapePvp : RmgLayoutFamily.NaturalLandscape) || (!Enum.IsDefined(settings.TerrainComplexity) ||
				(settings.GeneratorVersion is not (13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25) && settings.TerrainComplexity > Reassessment.TerrainComplexity.High))))
			...
			if (!Enum.IsDefined(settings.WaterAmount) || !Enum.IsDefined(settings.TacticalTerrain) ||
				(settings.GeneratorVersion is not (14 or 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25) && (settings.WaterAmount > RmgParameterLevel.High ||
			...
			settings.NeutralColonyWeights.Validate(settings.GeneratorVersion is 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 ? 100 : 1000);
			RmgColonyOwnership.ValidateShares(settings.StartingColonyShares, settings.PlayerCount);
			if (!settings.RespectStartingSafeArea && settings.GeneratorVersion is not (16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25))
				throw new ArgumentException("Starting safe area options require a current layout.");
			RmgArchipelagoParameters.Validate(settings);
			RmgChaosParameters.Validate(settings);
			if (settings.GeneratorVersion == 23) RmgLabyrinthParameters.Validate(settings);
			if (settings.GeneratorVersion != 23 && settings.ExtraRoutes != RmgLabyrinthRoutes.Standard) throw new ArgumentException("Extra Routes applies only to Labyrinth.");
			...
			if (!Enum.IsDefined(settings.StartingColonyMode) || (settings.GeneratorVersion is not (16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25) && settings.StartingColonyMode != RmgColonyOwnershipMode.ClosestToSpawn))
				throw new ArgumentException("Starting colony mode requires Regions V16 and a valid choice.");
			if (settings.GeneratorVersion is not (16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25) && settings.StartingColonyShares.Length != 0)
				throw new ArgumentException("Starting colony shares require Regions V16.");
			if (settings.GeneratorVersion is 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25)
			{
				...
				if (settings.GeneratorVersion == 23) { minimum = (minimum + 3) / 4; maximum = (maximum + 3) / 4; }
