...
namespace OpenRA.Mods.OpenSA.Rmg
{
	public static partial class RmgGenerator
	{
		...
		static void ValidateSettings(RmgProfile profile, RmgGenerationSettings settings)
		{
			...
			if (settings.GeneratorVersion is 17 or 18) RmgMirroring.Validate(settings.MirroringAxes, settings.MapSize, settings.PlayerCount);
			...
			if (!RmgBiome.IsSupported(settings.Tileset) || settings.Tileset != profile.Tileset ||
				(settings.Tileset != "NORMAL" && (settings.GeneratorVersion is not (16 or 17 or 18) || !profile.UsesRegionsTerrain)))
			...
			if (profile.UsesRegionsTerrain && (settings.TopologyPreset != (settings.GeneratorVersion == 18 ? RmgTopologyPreset.PlannedBattlefield : RmgTopologyPreset.NaturalRegions) ||
				settings.LayoutFamily != (settings.GeneratorVersion == 18 ? RmgLayoutFamily.ArtificialBattlefield : settings.GeneratorVersion == 17 ? RmgLayoutFamily.NaturalLandscapePvp : RmgLayoutFamily.NaturalLandscape) || (!Enum.IsDefined(settings.TerrainComplexity) ||
				(settings.GeneratorVersion is not (13 or 14 or 15 or 16 or 17 or 18) && settings.TerrainComplexity > Reassessment.TerrainComplexity.High))))
			...
			if (!Enum.IsDefined(settings.WaterAmount) || !Enum.IsDefined(settings.TacticalTerrain) ||
				(settings.GeneratorVersion is not (14 or 15 or 16 or 17 or 18) && (settings.WaterAmount > RmgParameterLevel.High ||
			...
			settings.NeutralColonyWeights.Validate(settings.GeneratorVersion is 16 or 17 or 18 ? 100 : 1000);
			RmgColonyOwnership.ValidateShares(settings.StartingColonyShares, settings.PlayerCount);
			if (!Enum.IsDefined(settings.StartingColonyMode) || (settings.GeneratorVersion is not (16 or 17 or 18) && settings.StartingColonyMode != RmgColonyOwnershipMode.ClosestToSpawn))
				throw new ArgumentException("Starting colony mode requires Regions V16 and a valid choice.");
			if (settings.GeneratorVersion is not (16 or 17 or 18) && settings.StartingColonyShares.Length != 0)
