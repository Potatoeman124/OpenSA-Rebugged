...
namespace OpenRA.Mods.OpenSA.Rmg
{
	...
	public static class RmgBattlefieldParameters
	{
		...
		public static void ValidateOptions(RmgPlayerSettings settings)
		{
			...
			if (!settings.IsPlannedBattlefield && (settings.BlockShape != RmgBattlefieldBlockShape.CutCorners || (!settings.IsCrossroads && !settings.IsRing && !settings.IsLabyrinth && !settings.IsDividedLands && settings.LaneWidth != RmgBattlefieldLaneWidth.Standard)))
