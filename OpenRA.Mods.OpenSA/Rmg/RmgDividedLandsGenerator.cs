	...
	public static partial class RmgGenerator
	{
		...
		static BattlefieldPlan PlanDividedLands(RmgProfile profile, RmgGenerationSettings settings)
		{
			...
			for (var i = 0; i < clear.Length; i++)
			{
				...
				// Fixed home areas leave the entire maximum channel envelope free of colonies.
				placement[i] = distance >= geometry.MaxChannelHalf + 2;
			}
			...
			for (var y = 0; y < size; y++)
				for (var x = 0; x < size; x++)
					blank.NativeTerrainIntents[4 * (y / 2 * (size / 2) + x / 2) + 2 * (y % 2) + x % 2] =
						placement[y * size + x] ? RmgNativeTerrainIntent.Clear : RmgNativeTerrainIntent.Water;
			var candidates = Enumerable.Range(0, size * size)
				...
				.OrderBy(o => TerrainComparison.Mix(settings.Seed, (ulong)(2101 + o[0].Y * size + o[0].X))).ToArray();
			...
			var report = new JObject
			{
				...
				["placement_order"] = "SEEDED_HOME_TERRITORY_SITES", ["fairness_scope"] = "EQUAL_TYPED_HOME_OPPORTUNITIES_AND_TERRAIN_COSTS_BEFORE_OWNERSHIP"
			};
		...
		static TerrainComparisonFields DividedLandsFields(RmgGenerationSettings settings, DividedLandsGeometry geometry, bool[] clear, bool[] land)
		{
			...
			var phase = TerrainComparison.Mix(settings.Seed, 2110) % 1024 / 1024D * Math.PI;
			double Variation(double x, double y) => .80 + (.02 + depth * .04) * Math.Cos(geometry.Along(x, y) / size * Math.PI * (3 + depth * 2) + phase);
			...
			for (var i = 0; i < water.Length; i++)
			{
				...
				var distance = geometry.Distance(x, y); var variation = Variation(x, y);
