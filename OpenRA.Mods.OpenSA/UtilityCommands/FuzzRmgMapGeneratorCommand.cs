...
namespace OpenRA.Mods.OpenSA.UtilityCommands
{
	sealed class FuzzRmgMapGeneratorCommand : IUtilityCommand
	{
		...
		[Desc("REPORT.json", "[--seed-start N]", "[--gate-a-count N]", "[--mixed-count N]",
			...
			"[--topology off|mixed|shoreline|land-details|land-cover]", "[--runtime-sample-rate N]", "[--preserve-failures]", "[--overwrite]",
		...
		void IUtilityCommand.Run(Utility utility, string[] args)
		{
			try
			{
				...
				{
					["schema_version"] = profile.GeneratorVersion >= 5 ? 6 : profile.GeneratorVersion >= 4 ? 5 : 4,
