...
namespace OpenRA.Mods.OpenSA.UtilityCommands
{
	sealed class GenerateRmgMapCommand : IUtilityCommand
	{
		...
		[Desc("OUTPUT.oramap", "--seed N | --player-settings FILE", "[--players 2|4]", "[--size 128,128|256,256]", "[--symmetry horizontal|vertical|rotational]", "[--archetype open|central-contest]", "[--topology off|mixed|shoreline|land-details|land-cover|battlefield-layout|parameterized-battlefield|coherent-water|natural-terrain|natural-terrain-v10]", "[--neutral-colonies N]", "[--water-amount low|standard|high]", "[--tactical-terrain low|standard|high]", "[--movement-validation proxy|native|both]", "[--report FILE]", "[--verify-repeatability]", "[--overwrite]", "Generate a deterministic OpenSA skirmish map. Schemas 1-4 preserve historical layouts; schema 5 preserves Regions V11; schema 6 preserves continuous Regions V12; schema 7 preserves Regions V13; schema 8 preserves V14 quantities/spacing; schema 9 preserves V15; schema 10 selects V16 with 64/128/256/512 square maps, 0-100 colony weights, starting_colony_shares, and optional starting_colony_mode (closest-to-spawn or random) and tileset (NORMAL, DESERT, SWAMP or CANDY).")]
		...
		static Options Parse(string[] args)
		{
			...
			if (values.TryGetValue("--tileset", out var tileset) && !string.Equals(tileset, "NORMAL", StringComparison.OrdinalIgnoreCase))
				throw new ArgumentException("The legacy command-line path supports only NORMAL. Use schema-10 --player-settings or Invoke-RegionsMapGenerator.ps1 -Tileset for other themes.");
