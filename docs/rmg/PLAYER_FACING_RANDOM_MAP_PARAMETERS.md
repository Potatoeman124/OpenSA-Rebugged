...
This document is a proposed player-facing contract for future in-game random map generation. It is not an implementation specification and does not make the current Generator Version 5 output ready for UI integration.
...
| --- | --- | --- | --- | --- |
| Generation preset | Dropdown | Balanced, Open Conflict, Tactical Crossroads, Narrow Passages | Sets a coherent combination of layout, Water, slow terrain, colonies, and chokepoints | Requires the layout phase |
| Players | Segmented buttons | 2, 4 | Number of supported starting positions | Supported now |
| Layout | Dropdown | Open Fields, Contested Center, Mixed Fronts, Narrow Passages | Controls battlefield regions, likely travel lanes, flank structure, and contest areas | Open/center prototypes exist; full contract required |
...
| Neutral colony density | Three-state selector | Sparse, Standard, Dense | Controls the number of capturable neutral colonies within legal values for the selected player count | Supported now through exact counts |
...
| Seed | Text box plus Randomize button | Unsigned numeric seed; generated by default | Reproduces the same map when all other settings and generator version match | Supported now |
...
| --- | --- | --- | --- |
| Symmetry | Dropdown | Automatic, Horizontal, Vertical, Rotational | Automatic should choose from legal modes. Explicit selection is useful for competitive play and reproducibility. |
