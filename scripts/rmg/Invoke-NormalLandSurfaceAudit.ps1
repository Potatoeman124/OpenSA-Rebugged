...
{
	$OutputDirectory = Join-Path $root "artifacts\rmg\phase-6a-land-surface-audit\corpus"
}
...
try
{
	& $utility "sa" "--audit-rmg-normal-land-surfaces" $OutputDirectory
	if ($LASTEXITCODE -ne 0)
	{
		throw "RMG NORMAL land-surface audit failed with exit code $LASTEXITCODE."
	}
}
finally
{
...
Write-Host "RMG Phase 6A land-surface evidence written to $OutputDirectory" -ForegroundColor Green
