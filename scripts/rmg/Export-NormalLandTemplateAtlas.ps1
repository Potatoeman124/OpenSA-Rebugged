...
{
	$OutputDirectory = Join-Path $root "artifacts\rmg\phase-6a-land-surface-audit\visual-atlas"
}
...
try
{
	& $utility "sa" "--export-rmg-normal-land-atlas" $OutputDirectory
	if ($LASTEXITCODE -ne 0)
	{
		throw "NORMAL land atlas export failed with exit code $LASTEXITCODE."
	}
}
finally
{
...
Write-Host "Local-only Phase 6A land atlases written to $OutputDirectory" -ForegroundColor Green
