"""Generate and verify paired Regions V13 maps; use only the Python standard library.
...
def schedule():
    ...
    for size in (128, 256):
        groups.append(dict(seed='7320260907003', size=f'{size},{size}', players=4, water_amount='high',
                           gravel_moss_amount='high', neutral_colony_density='dense', original_surface_relations=True))
        groups.append(dict(seed='7320260907004', size=f'{size},{size}', players=2, water_amount='low',
                           gravel_moss_amount='low', neutral_colony_density='sparse', original_surface_relations=False))
    return [dict(id=f'{i:02}-{group["size"].split(",")[0]}-{level}',
                 settings=dict(schema_version=7, preset='balanced', layout_family='natural-landscape',
...
def verify(folder, manifest):
    ...
    for case in manifest:
        ...
        assert report['generator_version'] == 13 and report['native_validation']['accepted'], case['id']
    ...
    measurements = []
    for case in manifest:
        cells = arrays[case['id']]
        report = reports[case['id']]
        size = report['size']
        water_edges = sum((cells[y*size+x] == 1) != (cells[y*size+x+1] == 1)
                          for y in range(size) for x in range(size-1)) + sum(
                          (cells[y*size+x] == 1) != (cells[(y+1)*size+x] == 1)
                          for y in range(size-1) for x in range(size))
        measurements.append(dict(id=case['id'], water_boundary_edges=water_edges,
            water_percent=100*cells.count(1)/len(cells),
            gravel_percent_land=100*cells.count(2)/(len(cells)-cells.count(1)),
            moss_percent_land=100*cells.count(3)/(len(cells)-cells.count(1)),
            colonies_requested=report['regions']['neutral_colonies_requested'],
            colonies_placed=report['regions']['neutral_colonies_placed'],
            start_displacement_native=report['regions']['start_displacement_native'],
            performance=report['performance']))
    (folder / 'measurements.json').write_text(json.dumps(measurements, indent=2)+'\n', encoding='utf-8')
    comparisons = []
    for offset in range(0, len(manifest), len(LEVELS)):
        triple = manifest[offset:offset + len(LEVELS)]
    ...
    print(f'Verified {len(manifest)} saved V13 maps, deterministic repeats, native terrain and actors.')


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('output', type=Path)
    parser.add_argument('--prototype', action='store_true', help='First two fixed seed/size groups for calibration.')
    ...
    if args.verify_only:
        ...
        manifest = schedule()[:10] if args.prototype else schedule()
