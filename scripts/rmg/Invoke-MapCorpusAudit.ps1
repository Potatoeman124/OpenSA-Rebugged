...
{
    $OutputDirectory = Join-Path $root "artifacts\rmg\phase-1-existing-map-audit\map_audit"
}
...
try
{
    & $utility "sa" "--audit-rmg-map-corpus" $OutputDirectory
    if ($LASTEXITCODE -ne 0)
    {
        throw "RMG map-corpus audit failed with exit code $LASTEXITCODE."
    }
}
finally
{
...
Write-Host "RMG Phase 1 audit data written to $OutputDirectory" -ForegroundColor Green
